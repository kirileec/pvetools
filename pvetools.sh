...
if [ $currentDebianVersion -gt 10 ];then
    securitySource="
deb https://mirror.nju.edu.cn/debian-security $sver-security main contrib non-free non-free-firmware
deb-src https://mirror.nju.edu.cn/debian-security $sver-security main contrib non-free non-free-firmware
"
else
    securitySource="
...
"
    ...
    #"a" "无脑模式" \
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config apt source:" 25 60 15 \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置apt镜像源:" 25 60 15 \
    "b" "更换为国内源" \
    "c" "关闭企业更新源" \
    "d" "还原配置" \
    "q" "返回主菜单" \
...
if [ $exitstatus = 0 ]; then
    case "$OPTION" in
a | A )
    if (whiptail --title "Yes/No Box" --yesno "修改为mirror.nju.edu.cn源，禁用企业订阅更新源，添加非订阅更新源(mirror.nju.edu.cn),修改ceph镜像更新源" 10 60) then
        if [ `grep "nju.edu.cn" /etc/apt/sources.list|wc -l` = 0 ];then
...
deb https://mirror.nju.edu.cn/debian/ $sver main contrib non-free non-free-firmware
deb-src https://mirror.nju.edu.cn/debian/ $sver main contrib non-free non-free-firmware
deb https://mirror.nju.edu.cn/debian/ $sver-updates main contrib non-free non-free-firmware
deb-src https://mirror.nju.edu.cn/debian/ $sver-updates main contrib non-free non-free-firmware
deb https://mirror.nju.edu.cn/debian/ $sver-backports main contrib non-free non-free-firmware
deb-src https://mirror.nju.edu.cn/debian/ $sver-backports main contrib non-free non-free-firmware
$securitySource
EOF
            #修改pve 5.x更新源地址为非订阅更新源，不使用企业订阅更新源。
            echo "deb https://mirror.nju.edu.cn/proxmox/debian/pve/ $sver pve-no-subscription" > /etc/apt/sources.list.d/pve-no-sub.list
            #关闭pve 5.x企业订阅更新源
            sed -i 's|deb|#deb|' /etc/apt/sources.list.d/pve-enterprise.list
            #修改 ceph镜像更新源
            #echo "deb https://mirror.nju.edu.cn/proxmox/debian/ceph-luminous $sver main" > /etc/apt/sources.list.d/ceph.list
...
软件源已更换成功！" 10 60
            ...
            whiptail --title "Success" --msgbox " Already changed apt source to nju.edu.cn!
已经更换apt源为 nju.edu.cn" 10 60
        fi
        if [ ! $1 ];then
            chSource
        fi
	...
	b | B  )
        ...
        if [ $exitstatus = 0 ]; then
            case "$OPTION" in
                a )
                    ss="mirrors.aliyun.com"
                    ;;
                b)
                    ss="mirror.nju.edu.cn"
                    ;;
                q )
                    chSource
            esac
            if (whiptail --title "Yes/No Box" --yesno "修改更新源为$ss?" 10 60) then
                if [ `grep $ss /etc/apt/sources.list|wc -l` = 0 ];then
                    ...
                    #sver=`cat /etc/apt/sources.list|awk 'NR==1{print $3}'`
                    cat > /etc/apt/sources.list << EOF
deb https://$ss/debian/ $sver main contrib non-free
deb-src https://$ss/debian/ $sver main contrib non-free
deb https://$ss/debian/ $sver-updates main contrib non-free
deb-src https://$ss/debian/ $sver-updates main contrib non-free
deb https://$ss/debian/ $sver-backports main contrib non-free
deb-src https://$ss/debian/ $sver-backports main contrib non-free
$securitySource
EOF
                    #修改 ceph镜像更新源
                    ...
                    echo "deb http://mirror.nju.edu.cn/proxmox/debian/pve/ $sver pve-no-subscription" > /etc/apt/sources.list.d/pve-no-sub.list
                    whiptail --title "Success" --msgbox " apt source has been changed successfully!
                    软件源已更换成功！" 10 60
                    apt-get update
                    apt-get -y install net-tools
...
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config samba:" 25 60 15 \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置samba:" 25 60 15 \
...
配置成功！
    ...
    d )
        smbRecycle(){
            if [ $L = "en" ];then
                x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config samba recycle:" 12 60 4 \
                "a" "Enable samba recycle." \
                "b" "Disable samba recycle." \
                "c" "Clear recycle." \
                3>&1 1>&2 2>&3)
            else
                x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置samba回收站！" 12 60 4 \
                "a" "开启samba回收站。" \
                "b" "关闭samba回收站。" \
                "c" "清空samba回收站。" \
                3>&1 1>&2 2>&3)
...
#config vim
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config VIM:" 12 60 4 \
    "a" "Install vim & simply config display." \
    "b" "Install vim & config 'vim-for-server'." \
    "c" "Uninstall." \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "安装配置VIM！" 12 60 4 \
    "a" "安装VIM并简单配置，如配色行号等。" \
...
yes or no?
            ...
            case $x in
                2 )
                    ...
                    cp ~/.vimrc ~/.vimrc.bak.pvetools
                    curl https://mirror.ghproxy.com/https://raw.githubusercontent.com/wklken/vim-for-server/master/vimrc > ~/.vimrc
                    whiptail --title "Success" --msgbox "
            Install & config complete!
            安装配置完成！
                    " 10 60
                ...
                1 | "" )
                    ...
                    git clone https://mirror.ghproxy.com/https://github.com/wklken/vim-for-server.git
...
doSpindown(){
    ...
    " 10 60) then
        ...
        cd /root
        git clone https://mirror.ghproxy.com/https://github.com/ivanhao/hdspindown.git
    {
        echo 10
        echo 50
        echo 90
...
chApm(){
    clear
    apm=$(
    whiptail --title " PveTools   Version : 2.5.0 " --menu "Config hard disks APM & AAM:
配置硬盘静音、降温：
    " 25 60 15 \
...
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config hard disks spindown:" 25 60 15 \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置硬盘自动休眠" 25 60 15 \
    "a" "配置硬盘自动休眠" \
    "b" "还原硬盘自动休眠配置" \
...
#-------------chCpu--main---------------
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config Cpufrequtils:" 25 60 15 \
    "a" "Config cpufrequtils to save power." \
    "b" "Remove config." \
    3>&1 1>&2 2>&3)
else
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "安装配置CPU省电" 25 60 15 \
    "a" "安装配置CPU省电(动态调整)" \
    "b" "还原配置" \
    3>&1 1>&2 2>&3)
fi
...
clear
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config subscribe:" 25 60 15 \
    "a" "Remove subscribe notice." \
    "b" "Unset config." \
    "c" "fix proxmox-widget-toolkit" \
    3>&1 1>&2 2>&3)
else
    OPTION=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置订阅提示" 25 60 15 \
    "a" "去除订阅提示" \
    "b" "还原配置" \
    "c" "修复去除订阅失败" \
    3>&1 1>&2 2>&3)
...
是否去除订阅提示?
            ...
            if [ `grep "data.status.toLowerCase() !== 'active'" /usr/share/javascript/proxmox-widget-toolkit/proxmoxlib.js|wc -l` -gt 0 ];then
                sed -i.bak "s/data.status.toLowerCase() !== 'active'/data.status.toLowerCase() === 'active'/g" /usr/share/javascript/proxmox-widget-toolkit/proxmoxlib.js
...
unsetVmN(){
    ...
    vmid=$(whiptail  --title " PveTools   Version : 2.5.0 " --menu "
...
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config Nested:" 25 60 15 \
    "a" "Enable nested" \
    "b" "Set vm to nested" \
    "c" "Unset vm nested" \
    "d" "Disable nested" \
    ...
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置嵌套虚拟化:" 25 60 15 \
    "a" "开启嵌套虚拟化" \
    "b" "开启某个虚拟机的嵌套虚拟化" \
    "c" "关闭某个虚拟机的嵌套虚拟化" \
    "d" "关闭嵌套虚拟化" \
...
clear
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config lm-sensors & proxmox ve display:" 25 60 15 \
    "a" "Install." \
    "b" "Uninstall." \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置Sensors:" 25 60 15 \
    "a" "安装配置温度、CPU频率显示" \
    "b" "删除配置" \
    3>&1 1>&2 2>&3)
fi
...
$(echo $cards) \
    ...
    if [ $exitstatus = 0 ];then
        ...
        if [ $DISTROS ];then
            ...
            list1=`echo $list|awk 'NR>1{print $1}'`
            vmid=$(whiptail  --title " PveTools   Version : 2.5.0 " --radiolist "
...
您已经配置过这个了!
                    ...
                    opt=$(whiptail  --title " PveTools   Version : 2.5.0 " --checklist "
Choose options:
选择选项：" 20 60 10 \
...
configVideo(){
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config PCI Video card Passthrough:" 25 60 15 \
    "a" "Config Video Card Passthrough" \
    "b" "Config Video Card Passthrough to vm" \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置PCI显卡直通:" 25 60 15 \
    "a" "配置物理机显卡直通支持。" \
    "b" "配置显卡直通给虚拟机。" \
    3>&1 1>&2 2>&3)
fi
...
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config PCI Passthrough:" 25 60 15 \
    "a" "Config IOMMU on." \
    "b" "Config IOMMU off." \
    ...
    x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置硬件直通:" 25 60 15 \
    "a" "配置开启物理机硬件直通支持。" \
    "b" "配置关闭物理机硬件直通支持。" \
    "c" "配置显卡直通。" \
    "d" "配置qm set 硬盘给虚拟机。" \
...
安装配置完成！" 10 60
    ...
    enterChroot(){
        ...
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Enter chroot:" 25 60 15 \
            $(echo $c) \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "进入chroot环境:" 25 60 15 \
            $(echo $c) \
            3>&1 1>&2 2>&3)
        fi
        exitstatus=$?
...
chroot后台运行环境已经运行，需要重启吗？
    ...
    configChroot(){
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config chroot & docker etc:" 25 60 15 \
            ...
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置chroot环境和docker等:" 25 60 15 \
...
输入格式错误，请重新输入：
                ...
                if [ $1 = 'add' ];then
                    ...
                    d=$(whiptail --title " PveTools Version : 2.5.0 " --checklist "
disk list:
已添加的硬盘:
...
选择硬盘：" 30 90 10 \
                    ...
                    3>&1 1>&2 2>&3)
                    exitstatus=$?
                    t=$(whiptail --title " PveTools Version : 2.5.0 " --menu "
Choose disk type:
选择硬盘接口类型：" 20 60 10 \
                    "sata" "vm sata type" \
                    "scsi" "vm scsi type" \
...
配置完成" 10 60
    ...
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Config qm set disks:" 25 60 15 \
        "a" "set disk to vm." \
        "b" "unset disk to vm." \
        3>&1 1>&2 2>&3)
    else
        x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "配置qm set 物理硬盘给虚拟机:" 25 60 15 \
        "a" "添加硬盘给虚拟机。" \
        "b" "删除虚拟机里的硬盘。" \
        3>&1 1>&2 2>&3)
    fi
...
$maps
    ...
    setDns(){
        ...
        if [ `cat /etc/resolv.conf|grep 'nameserver'|wc -l` != 0 ];then
            if [ $L = "en" ];then
                d=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "DNS - Many Tools:
Detect exist nameserver,Please choose:
                ...
                d=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "DNS - 常用的工具:
检测到已经配置有dns服务器: \
...
请选择以下操作：
        ...
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "DNS - Many Tools:" 25 60 15 \
            "a" "8.8.8.8(google)." \
            "b" "223.5.5.5(alidns)." \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "DNS - 常用的工具:" 25 60 15 \
            "a" "8.8.8.8(谷歌)." \
            "b" "223.5.5.5(阿里)." \
            3>&1 1>&2 2>&3)
        fi
...
释放内存？" 10 60 );then
    ...
    darkMode(){
        if [ $L = "en" ];then
            d=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "DarkMode - Many Tools:
            " 25 60 15 \
            "a" "Install." \
            "b" "Uninstall." \
            3>&1 1>&2 2>&3)
...
#----------------- 请选择以下操作：----------------- \
            d=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "界面黑暗模式 - 常用的工具:
            " 25 60 15 \
            "a" "安装." \
            "b" "卸载." \
            3>&1 1>&2 2>&3)
...
提取显卡vbios失败。" 10 60
    ...
    folder2ram(){
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "folder2ram:" 25 60 15 \
            "a" "install" \
            "b" "Uninstall" \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "USB设备做为系统盘的优化:" 25 60 15 \
            "a" "安装。" \
            "b" "卸载。" \
            3>&1 1>&2 2>&3)
        fi
...
配置完成" 10 60
    ...
    autoResize(){
        if [ $L = "en" ];then
            d=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "autoResize ROOT partition - Many Tools:
            " 25 60 15 \
            "a" "start." \
            3>&1 1>&2 2>&3)
        else
#----------------- 请选择以下操作：----------------- \
            d=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "自动扩展ROOT分区可用空间 - 常用的工具:
            " 25 60 15 \
            "a" "运行." \
            3>&1 1>&2 2>&3)
        fi
    ...
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Many Tools:" 25 60 15 \
        ...
        x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "常用的工具:" 25 60 15 \
        "a" "局域网扫描。" \
        "b" "配置DNS。" \
        "c" "释放内存。" \
        "d" "speedtest测速" \
...
chNFS(){
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "NFS:" 25 60 15 \
...
sambaOrNfs(){
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.5.0 " --menu "Samba or NFS:" 25 60 15 \
        "a" "samba." \
        "b" "NFS" \
        3>&1 1>&2 2>&3)
    else
